...
def moveFile(path_before, path_after, selectNumber):
    ...
    for file in filelist:
        ...
        if (selectNumber == True):
            ...
            target = dirAdrs +"\\"+ file
            # forCSV.append(ai.getFileData(target))
            ...
            dict[file]=temp_list[-1]
        Log.selectOrganizeLog(file, path_before, path_after)
    #딕셔너리 정보 활용하여 파일 이동
    cnt = 0
    for key, value in dict.items():
        shutil.move(path_before+"\\"+key, path_after+"\\"+value)
        forCSV.append(ai.getFileData(path_after+"\\"+value))
        cnt +=1
    ai.makeCsv(forCSV, 'data.csv')
    return cnt
